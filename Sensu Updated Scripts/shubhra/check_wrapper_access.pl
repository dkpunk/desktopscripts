...
my $command = `$my_ps aux | $my_grep -w defunct | $my_grep -w check_access.pl | $my_grep -v grep | $my_awk '{print \$2}' `;
my @process_details = split("\n",$command);
if(@process_details){
        foreach (@process_details){
                my $temp_ps =  `ps -ef | grep -w $_ | grep -w check_access.pl | grep defunct | grep -v grep` ;
		if($temp_ps){
		#my $search = `$my_ps -ef | $my_grep -w $_ | $my_grep -v grep | $my_awk '{print \$2" " \$3}' | $my_xargs echo`;
                 my $search = `$my_ps -ef | $my_grep -w $_ | grep -w check_access.pl | $my_grep -v grep | $my_awk '{print \$2" " \$3}' | $my_xargs kill -9`;
			sleep(0.5);
		}
        }
}
